...
func (e *engine) open(path string) (bool, error) {
	...
	if err != nil && !os.IsExist(err) {
		...
		err = createEmptyFile(path+`.ix`, startPage)
	...
	// read meta file and set e.page/block size
	info, err = os.Stat(path + `.ix`)
	if err != nil {
		return fdstat, err
	}
	// set e.page/block size
	e.page = int(info.Size())
	// set / reassign maxVal size
	e.maxVal = e.page - maxKey - 1
	e.empty = make([]byte, e.page, e.page)
...
func (e *engine) addRecord(r *record) (int, error) {
	...
	for o < len(e.data) {
		...
		if bytes.Equal(e.data[o:o+e.page], e.empty) {
...
func (e *engine) setRecord(k int, r *record) error {
	...
	copy(e.data[o:o+e.page], e.empty)
